...
Eventually, we will use this method (and this exact database) to add authentication to a page that you previously created in [a01](assignment/a01).
...
We are also going to use [Express.js](TODO), which is a very simple webserver, to get us going without too much setup. 
...
We'll start by installing [Express.js](TODO), which we will use as a web server for our API.
...
This is a package that allows for the hashing of strings using the [MD5 algorithm](TODO).
...
Finally, we need a database. We are going to use a JS implementation of a [sqlite3]() database.
...
You will have only two main files for this assignment, `server.js` which contains the bulk of our code to create the API and `database.js` which deals with the database.
Skeletons of these files have been created for you.
...
If there is not, then it will create one and populate it with two users.
...
This should return every record held in the table created by `database.js`.
...
Look at the [better-sqlite *statements* class](https://github.com/JoshuaWise/better-sqlite3/blob/HEAD/docs/api.md#class-statement) under `all()` and `get()` to find the relevant functions for our purposes.
...
For this assignment, you need to define 5 endpoints for your API.
